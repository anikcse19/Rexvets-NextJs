...
const appointmentSchema = new Schema<IAppointment>(
  {
    ...
    appointmentTime: {
      ...
      validate: {
        ...
        message:
          "Appointment time must be a valid time format (HH:MM or HH:MM AM/PM)",
      },
    },
    status: {
      type: String,
    ...
    callDuration: {
      ...
      validate: {
        ...
        message:
          'Call duration must be in format like "0m 9s", "1h 30m", or "45m"',
      },
    },
    videoCallCompleted: {
      type: Boolean,
...
appointmentSchema.virtual("isUpcoming").get(function () {
  ...
  const appointmentDateTime = new Date(
    `${this.appointmentDate}T${this.appointmentTime}`
  );
...
appointmentSchema.virtual("durationMinutes").get(function () {
  if (this.callDuration) {
    ...
    const match = this.callDuration.match(
      /(?:(\d+)h\s*)?(?:(\d+)m\s*)?(?:(\d+)s\s*)?/
    );
    if (match) {
...
appointmentSchema.statics.findByVeterinarianId = function (
  veterinarianId: mongoose.Types.ObjectId
) {
  return this.find({ veterinarianId }).sort({
    appointmentDate: 1,
    appointmentTime: 1,
  });
};
...
appointmentSchema.statics.findByParentId = function (
  parentId: mongoose.Types.ObjectId
) {
  return this.find({ parentId }).sort({
    appointmentDate: 1,
    appointmentTime: 1,
  });
};
...
appointmentSchema.statics.findUpcomingAppointments = function (
  userId: mongoose.Types.ObjectId,
  userType: "veterinarian" | "petParent"
) {
  const query =
...
appointmentSchema.statics.findPastAppointments = function (
  userId: mongoose.Types.ObjectId,
  userType: "veterinarian" | "petParent"
) {
  const query =
...
appointmentSchema.statics.findTodayAppointments = function (
  userId: mongoose.Types.ObjectId,
  userType: "veterinarian" | "petParent"
) {
  const query =
...
appointmentSchema.statics.findAppointmentsByDateRange = function (
  userId: mongoose.Types.ObjectId,
  userType: "veterinarian" | "petParent",
  startDate: string,
  endDate: string
...
appointmentSchema.statics.findAppointmentsByStatus = function (
  userId: mongoose.Types.ObjectId,
  userType: "veterinarian" | "petParent",
  status: string
) {
