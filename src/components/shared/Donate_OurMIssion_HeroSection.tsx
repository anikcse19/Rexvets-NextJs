...
const Donate_OurMIssion_HeroSection: FC<HeroSectionProps> = () => {
  ...
  const data = {
    ...
    title: "Our Mission",
    subtitle:
      "At Rex Vet, our mission is to increase access to veterinary care for all pets—regardless of their family's financial or geographic limitations.",
    description:
      "We believe that early and consistent veterinary care is essential for animal health and critical to strengthening the human-animal bond. Together, we can make a significant difference in the lives of pets and their families.",
    primaryButton: {
      text: "Donate Now",
    ...
    secondaryButton: {
      ...
      onClick: () => router.push("/find-a-vet"),
    },
  ...
  return (
    <section
      className={`relative min-h-screen flex items-center py-12 md:py-0 bg-gradient-to-br ${data.backgroundGradient} overflow-hidden ${data.className}`}
    >
      {/* Animated background elements */}
      {data?.showAnimatedBackground && (
        <>
          <motion.div
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <motion.h1
            ...
            className={`text-4xl md:text-6xl lg:text-[96px] font-black mb-6 text-shadow-lg  text-white bg-clip-text`}
          >
            {data.title}
          </motion.h1>
          ...
          >
            {splitString(data.subtitle) ? (
              <>
                <span>{splitString(data.subtitle)?.first}</span>
                <span className=" ml-1 text-[#41ABF3]">
                  {splitString(data.subtitle)?.second}
                </span>
              </>
            ) : (
              data.subtitle
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-center">
              ...
              {data.secondaryButton && (
                <button
                  ...
                  className="group relative flex cursor-pointer items-center justify-center px-8 py-5 text-white font-semibold text-lg rounded-2xl border-2 border-blue-400 hover:border-blue-300 hover:bg-blue-500/10 transform hover:scale-105 transition-all duration-300 shadow-lg backdrop-blur-sm"
                >
                  {data.secondaryButton.icon}
                  {data.secondaryButton.text}

                  {/* Overlay behind content */}
                  <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-transparent via-blue-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10"></div>
                </button>
              )}
            </div>
          </motion.div>
