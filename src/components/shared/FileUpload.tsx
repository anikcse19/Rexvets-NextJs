    ...
    if (file.type.startsWith("image/") && file.size > 1024 * 1024) {
      ...
      try {
        ...
        if (
          compressedFile &&
          compressedFile.name &&
          typeof compressedFile.size === "number"
        ) {
  ...
  const validateAndProcessFile = useCallback(
    async (file: File): Promise<FileWithPreview> => {
      ...
      if (
        !renamedFile ||
        !renamedFile.name ||
        typeof renamedFile.size !== "number"
      ) {
        console.error("Renamed file validation failed:", renamedFile);
        throw new Error(
          "File processing failed - invalid file object returned"
        );
      }
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
          ...
          try {
            ...
            if (!processedFile.name || typeof processedFile.size !== "number") {
              errors.push(
                `${file.name}: File processing failed - invalid file object`
              );
              continue;
            }
