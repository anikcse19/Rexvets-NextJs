...
export default function DonationPage() {
  ...
  const handleEditDonation = async () => {
    ...
    const donation = donations.find(
      (d) => (d as any)._id === selectedDonationId
    );
    if (!donation) return;
  ...
  const handleDeleteDonation = async () => {
    ...
    try {
      const response = await fetch(
        `/api/donations/${(donationToDelete as any)._id}`,
        {
          method: "DELETE",
        }
      );
        ...
        <Card className="dark:bg-slate-800 bg-gray-100">
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Input
              ...
              value={filters.donor}
              onChange={(e) =>
                setFilters({ ...filters, donor: e.target.value })
              }
                    ...
                    <TableRow key={index}>
                      ...
                      <TableCell className="text-right space-x-2">
                        ...
                        <Dialog
                          open={
                            editDialogOpen &&
                            selectedDonationId === (donation as any)._id
                          }
                          onOpenChange={(open) => {
