...
export default function AdminModeratorsPage() {
  ...
  const handleAddModerator = async () => {
    ...
    try {
      ...
      if (data.tempPassword) {
        toast.info(`Password for ${form.email}: ${data.tempPassword}`);
      } else if (form.password) {
        toast.error(`Password for ${form.email}: ${form.password}`);
      }
    } catch (error) {
    ...
    if (selectedModeratorId !== null) {
      try {
        const response = await fetch(
          `/api/admin/moderators/${selectedModeratorId}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDeleteModerator = async () => {
    if (moderatorToDelete) {
      try {
        const response = await fetch(
          `/api/admin/moderators/${moderatorToDelete._id}`,
          {
            method: "DELETE",
          }
        );
                  ...
                  moderators.map((mod, index) => (
                    <TableRow key={mod._id} className="dark:hover:bg-slate-700">
                      ...
                      <TableCell className="text-right space-x-2">
                        <Dialog
                          open={
                            editDialogOpen && selectedModeratorId === mod._id
                          }
                          onOpenChange={(open) => {
                          ...
                          <DialogContent>
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                ...
                                <strong>{mod.name}</strong>? This action cannot
                                be undone.
                              </DialogDescription>
                            </DialogHeader>
