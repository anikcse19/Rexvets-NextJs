...
const PrescriptionPDF = ({
  ...
  // console.log(signatureUrl, "sign", doctor, "doctr");
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.doctorTitle}>
            DR. {doctor.FirstName + " " + doctor.LastName || "N/A"}
          </Text>
          <View style={styles.rowLine}>
            <Text style={styles.label}>Phone:</Text>
            <Text style={styles.value}>{doctor?.PhoneNo || "-"}</Text>
          </View>

          <View style={styles.rowLine}>
            <Text style={styles.label}>Email:</Text>
            <Text style={styles.value}>{doctor.EmailAddress || "-"}</Text>
          </View>

          <View style={styles.rowLine}>
            <Text style={styles.label}>License #:</Text>
            <Text style={styles.value}>
              {doctor?.urls?.[0]?.licenseNumber || "-"}
            </Text>
          </View>
          <Text style={styles.text}>DEA #: {doctor?.urls?.dea || "-"}</Text>
        </View>
          ...
          <View style={styles.row}>
            <View style={styles.col}>
              ...
              <View style={styles.row}>
                ...
                <Text style={styles.value}>{parent?.State || "-"}</Text>
              </View>

              <View style={styles.row}>
                <Text style={styles.label}>Phone:</Text>
                <Text style={styles.value}>{parent?.PhoneNo || "-"}</Text>
              </View>
              <View style={styles.row}>
                <Text style={styles.label}>Email:</Text>
                <Text style={styles.value}>{parent?.EmailAddress || "-"}</Text>
              </View>
            </View>
            <View style={styles.col}>
              ...
              <View style={styles.row}>
                ...
                <Text style={styles.value}>
                  {pet?.AgeYears || "-"} yrs, {pet?.AgeMonths || "-"} mos
                </Text>
              </View>
            </View>
          </View>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.text}>
            {values.medicationName.toUpperCase()} {values.strength}
            {values.strengthUnit}, {values.form}
          </Text>
          <View style={styles.row}>
            <Text style={styles.label2}>Quantity:</Text>
            <Text style={styles.value}>{values.quantity}</Text>
          </View>
          <View style={styles.row}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.value}>{values.refills || 0}</Text>
          </View>
          <View></View>
          <View style={[styles.row, styles.gap]}>
            <Text style={styles.label2}>Generic Permissible?</Text>
            <Text style={styles.value}>
              {values.genericAllowed ? "Yes" : "No"}
            </Text>
          </View>
          <View style={styles.row}>
            <Text style={styles.label2}>Allow Human Pharmacy?</Text>
            <Text style={styles.value}>
              {values.allowHumanPharmacy ? "Yes" : "No"}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.text}>{values.directions}</Text>
        </View>

        {/* Note */}
        <View style={styles.section}>
          <Text style={styles.title}>Note to Pharmacist:</Text>
          <Text>{values.pharmacistNote}</Text>
        </View>

        {/* Signature */}
        <View style={styles.signatureSection}>
          {signatureUrl ? (
            <Image src={signatureUrl} style={{ width: 100 }} />
          ) : (
            <>
