  ...
  return (
    <div className="container mx-auto ">
      ...
      <Tabs defaultValue="history" className="space-y-6">
        <TabsList className="grid grid-cols-2 w-full sm:max-w-lg mx-auto bg-white shadow-sm border rounded-lg overflow-hidden">
          <TabsTrigger
            value="request"
            className="flex items-center gap-2 py-2 data-[state=active]:bg-blue-600 data-[state=active]:text-white"
          >
        ...
        {[
          {
            ...
            title: "Quick & Easy",
            text: "Submit your prescription transfer request in just a few minutes",
            bg: "bg-green-100",
          },
          {
            icon: <FileText className="h-6 w-6 text-purple-600" />,
            title: "Secure Payment",
            text: "Safe and encrypted payment processing with industry-standard security",
            bg: "bg-purple-100",
          },
          {
            icon: <History className="h-6 w-6 text-blue-600" />,
            title: "Track Progress",
            text: "Monitor the status of your transfer requests in real-time",
            bg: "bg-blue-100",
          },
        ].map((item, idx) => (
          <div
            key={idx}
            className="text-center p-6 bg-white rounded-xl shadow-sm border"
          >
            <div className={`p-3 ${item.bg} rounded-full w-fit mx-auto mb-4`}>
              {item.icon}
            </div>
