...
const PetParentHeroSection = () => {
    const chatMessages: Message[] = [
        ...
        { isVet: false, message: "She seems a bit lethargic and isn't eating much." },
        { isVet: false, message: "Just some occasional coughing." },
        { isVet: true, message: "Let's do a virtual check-up to see how we can help." },
    ];
    return (
        <section className="relative min-h-screen flex items-center bg-cover bg-center bg-fixed 
      before:absolute before:inset-0 before:bg-[linear-gradient(135deg,rgba(0,35,102,0.9)_0%,rgba(67,56,202,0.9)_50%,rgba(139,92,246,0.9)_100%)]"
            style={{ backgroundImage: "url('/images/pet-parents/membershipsbg1.webp')" }}>
            ...
            <div className="container relative z-10 mx-auto px-6 lg:px-12 grid lg:grid-cols-2 gap-12 items-center">
                ...
                <div className="text-white">
                    ...
                    <p className="mt-6  text-[1.1rem] md:text-[1.3rem] mb-6 text-white/90 leading-[1.6] drop-shadow-[0_2px_10px_rgba(0,0,0,0.2)] max-w-[600px] ">
                        At Rex Vets, we believe every pet deserves access to quality veterinary care, regardless of their owner's financial situation. Join our support community and help us continue providing essential care to pets everywhere.
                    </p>
                    <Link
                        href="/donate"
      ...
      rounded-3xl border border-white/20 bg-white/95 backdrop-blur-2xl px-6 py-7 shadow-2xl`}
            >
                <h3 className="mb-6 font-bold text-[#1e293b] text-xl">Live Consultation</h3>
                ...
                <div>
                    {chatMessages.map((msg, idx) => (
                        <motion.div
                            ...
                            className={`flex items-start gap-3 mb-4 ${msg.isVet ? "flex-row" : "flex-row-reverse"}`}
                        >
                            {msg.isVet && (
              ...
              ${msg.isVet ? "bg-slate-100 text-slate-800" : "bg-blue-500  text-white"}`}
                            >
                                {msg.message}
                            </div>
