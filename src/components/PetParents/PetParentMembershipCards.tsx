...
const cards = [
  {
    ...
    description:
      "Join as a Friend of Rex Vet and help us provide Affordable telehealth consultations to pets in need. Your monthly contribution ensures ongoing care for pets who otherwise might not receive it.",
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {cards.map((card, i) => {
          ...
          return (
            <div
              key={i}
              className={`relative rounded-2xl shadow-lg p-8 bg-white hover:scale-105 overflow-hidden transition ${
                card.recommended ? "border-3 border-[#f8bd55]" : "border"
              }`}
            >
              {/* recoimended section */}
              {card.recommended && (
              ...
              <p className="text-slate-500 font-normal mb-5 leading-[1.6]">
                {card.description}
              </p>
              <p className="font-bold mb-2">
                Suggested Contribution:<br></br>{" "}
              </p>
              <span className="text-slate-500 font-normal mb-4 leading-[1.6]">
                {card.contribution}
              </span>
              ...
              <ul className="space-y-2 my-6">
                {card.benefits.map((b, j) => (
                  <li
                    key={j}
                    className="flex items-center text-[14px] gap-2 text-slate-500 font-normal mb-4 leading-[1.6]"
                  >
              ...
              <Link
                ...
                className={`w-full inline-flex justify-center mt-5 items-center py-4 rounded-full font-bold shadow-xl 
                  ${
                    card.recommended
                      ? "bg-gradient-to-r from-[#f8bd55] to-[#f6a10d] text-black shadow-[#fff3d5] hover:shadow-[#fcecc3]"
                      : "bg-gradient-to-r from-blue-500 to-purple-500 text-white shadow-[#e1ecfe] hover:shadow-[#c8d9f5] "
